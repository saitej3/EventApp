...
class DB_Functions {
    ...
    public function storeUser($name, $email, $password) {
        ...
        $salt = $hash["salt"]; // salt
        $result = mysql_query("INSERT INTO users(id, name, email,password, salt, created_at) VALUES('$uuid', '$name', '$email', '$encrypted_password', '$salt', NOW())");
    ...
    public function registerEvent($name,$description,$date,$time,$contact_name_1,$contact_no_1,$contact_name_2,$contact_no_2,$Image,$requirements,$notes)
    {
        ...
        {
            $uid = mysql_insert_id(); // last inserted id
            $result = mysql_query("SELECT * FROM events WHERE uid = $uid");
            return mysql_fetch_array($result);
        }
        else
            return false;
